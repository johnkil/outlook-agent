...
| EWS enablement and live smoke validation | https://github.com/johnkil/outlook-agent/issues/6 | EWS read-metadata live smoke harness `TestLiveEWSReadMetadataSmoke` is implemented for `auth check` and metadata-only `GetFolder`. | The remaining gate needs approved endpoint/auth method, controlled live credential storage, successful `auth check`, and controlled read-metadata smoke evidence from a private run. |
