...
| enterprise distribution channel | https://github.com/johnkil/outlook-agent/issues/4 | The initial pilot channel is checksum-verified direct archive install from the latest approved GitHub release. Public release ownership and rollback ownership are documented in `docs/ENTERPRISE_ENABLEMENT.md`; `v0.1.1` publishes six portable archives and `SHA256SUMS.txt`. | Enterprise operators still need to name the private rollout owner, private config/profile owner, pilot group, and support path outside this public repository before enterprise rollout. |
