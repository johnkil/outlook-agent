...
Direct `setup agent` writes only the config path string into MCP arguments.
Wrapper mode keeps the config path in the wrapper generated by `setup approval`.
In both modes, setup does not read, copy, inline, or validate the private config file contents.
