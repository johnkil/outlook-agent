...
- Protocol breadth:
  ...
  - typed EWS high-level workflows beyond the current `GetFolder`,
    metadata-only `mail.search`, and metadata-only `mail.fetch_metadata` read
    paths plus raw `EWSRequest` escape hatch;
