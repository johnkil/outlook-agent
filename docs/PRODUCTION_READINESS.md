...
| Live verification | Partial | Authenticated discovery has sanitized evidence for the useful OWA app shell and 25 live-discovered actions; high-level mail search, availability, stdio MCP availability, read-only raw `FindPeople`, and representative MCP dry-run gates for reversible/destructive OWA actions have opt-in live smoke coverage. Full live execution of every raw action is intentionally not attempted because many actions are destructive, send-like, or settings-changing. |
...
- Live validation:
  ...
  - additional live dry-run summaries for send-like and settings/rules requests
    without executing send-like or settings-changing actions.
