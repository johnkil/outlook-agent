...
| High-level mail/calendar workflows | Ready initial set | Search, metadata fetch, explicit fixture body fetch, explicit attachment listing/fetch, draft save, reply/reply-all/forward draft save, typed send-draft, reversible message organization (`mail.move_to_folder`, `mail.archive`, `mail.flag`, `mail.categorize`, `mail.mark_read`), move to Deleted Items, read-only rules/settings metadata tools, the first carefully gated typed rule write helper `mail.rules.set_enabled`, calendar list, and availability are implemented. Live smoke evidence exists through controlled fixtures or bounded metadata reads where live transport policy allows it; Graph reply/forward drafts, send-draft, reversible organization actions, and rule set-enabled are unit/MCP-tested and remain outside the read-only live harness. |
