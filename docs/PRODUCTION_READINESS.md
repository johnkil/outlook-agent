...
| High-level mail/calendar workflows | Ready initial set | Search, metadata fetch, explicit fixture body fetch, explicit attachment fetch, draft save, move to Deleted Items, calendar list, and availability are implemented. All except attachment fetch have live smoke evidence; attachment fetch is unit-tested and needs a controlled live fixture before claiming live coverage. |
...
| Graph/EWS adapters | Partial | EWS has an initial SOAP adapter with config wiring and a read-metadata `GetFolder` auth probe/action covered by unit tests; the tested live endpoint returned an empty/EOF response before SOAP auth completed, so environment enablement or alternate auth remains unresolved. Graph has an initial bearer-token REST adapter with config wiring and high-level mail/calendar workflows (`GetMailFolder`, `mail.search`, `mail.fetch_metadata`, `mail.fetch_body`, `mail.fetch_attachment`, `mail.create_draft`, `mail.move_to_deleted_items`, `calendar.list`, `calendar.availability`) covered by unit tests; live Graph probing stopped at missing local Keychain token, so OAuth/admin-consent enablement remains unresolved. |
