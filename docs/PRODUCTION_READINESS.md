...
- Protocol breadth:
  ...
  - typed EWS high-level workflows beyond the current `GetFolder`,
    metadata-only `mail.search`, metadata-only `mail.fetch_metadata`, and
    metadata-only `calendar.list`, and metadata-only `calendar.availability`
    read paths plus raw `EWSRequest` escape hatch;
